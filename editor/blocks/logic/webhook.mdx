...
```
https://zazubot.com/api/v1/typebots/{typebotId}/blocks/{blockId}/web/executeTestWebhook
```

- WhatsApp:

```
https://zazubot.com/api/v1/typebots/{typebotId}/blocks/{blockId}/whatsapp/{phone}/executeTestWebhook
```
...
```
https://zazubot.com/api/v1/typebots/{typebotId}/blocks/{blockId}/results/{resultId}/executeWebhook
```

Where `{resultId}` is the current user result ID. You can get this ID directly in your zazubot flow using a [Set variable](/editor/blocks/logic/set-variable) block with the `Result ID` value.
